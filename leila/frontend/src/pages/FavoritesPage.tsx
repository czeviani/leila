...
const LIQUIDITY_CONFIG = {
  alta:  { label: 'Liquidez Alta',  color: 'text-emerald-600', bg: 'bg-emerald-50' },
  media: { label: 'Liquidez Média', color: 'text-amber-600',   bg: 'bg-amber-50'   },
  baixa: { label: 'Liquidez Baixa', color: 'text-red-600',     bg: 'bg-red-50'     },
}
  ...
  return (
    <div className="bg-slate-50 rounded-xl border border-slate-100 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        {liquidity && (
          <span className={`ml-auto text-[10px] font-semibold px-2 py-0.5 rounded-full ${liquidity.bg} ${liquidity.color}`}>
            {liquidity.label}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
        <div className="bg-white rounded-lg border border-slate-100 p-3">
          <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wider mb-1">Custo Total</p>
        ...
        {pricePerM2 && (
          <div className="bg-white rounded-lg border border-slate-100 p-3">
            <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wider mb-1">Preço/m²</p>
            <p className="text-sm font-bold text-slate-900">R$ {pricePerM2.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</p>
            {fd.price_vs_market_pct != null && (
              <p className={`text-[10px] mt-0.5 font-medium ${fd.price_vs_market_pct < 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                {fd.price_vs_market_pct < 0 ? '' : '+'}{fd.price_vs_market_pct.toFixed(1)}% vs. mercado
              </p>
            )}
          </div>
        )}
        ...
        {fd.market_avg_price_m2 && (
          <div className="bg-white rounded-lg border border-slate-100 p-3">
            <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wider mb-1">Mercado/m²</p>
        ...
        {fd.rental_yield_annual_pct != null && (
          <div className={`rounded-lg border p-3 ${fd.rental_yield_annual_pct >= 6 ? 'bg-emerald-50 border-emerald-100' : 'bg-white border-slate-100'}`}>
            <p className={`text-[10px] font-medium uppercase tracking-wider mb-1 ${fd.rental_yield_annual_pct >= 6 ? 'text-emerald-600' : 'text-slate-400'}`}>Yield Anual</p>
        ...
        {isPending && (
          <div className="flex items-center gap-3 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Isso pode levar alguns segundos. A página atualiza automaticamente.</p>
            </div>
          </div>
        )}
        ...
        {isDone && evaluation && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                { label: 'Localização', value: evaluation.location_notes, icon: MapPin },
                { label: 'Condições', value: evaluation.condition_notes, icon: Building2 },
                { label: 'Documentação', value: evaluation.documents_notes, icon: Home },
              ].filter(n => n.value).map(({ label, value, icon: Icon }) => (
                <div key={label} className="p-3.5 bg-slate-50 rounded-xl border border-slate-100">
            ...
            {(evaluation.highlights?.length > 0 || evaluation.risks?.length > 0) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {evaluation.highlights?.length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Pontos Positivos</p>
                ...
                {evaluation.risks?.length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Riscos</p>
                    <ul className="space-y-1.5">
          ...
          <div className="flex flex-col items-center justify-center py-28 gap-5">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-slate-400 mt-2 max-w-xs leading-relaxed">
                Selecione imóveis na aba <strong className="text-slate-600">Imóveis</strong> clicando no ícone de coração.
